...
function DNAButtonGroup(props) {
  const { urlTo, row, callback, moreActions } = props;
  let navigate = useNavigate();
  const dataService = new DataService(urlTo);
  ...
  const handleAction = (action) => {
    navigate(`/${urlTo}-form/${action}/${row.id}`);
  };
  ...
  return (
    <ButtonGroup variant="outlined" aria-label="botões para a tabela">
      <Tooltip title="Abrir formulário para visualizar os dados deste registro">
        <IconButton aria-label="visualizar" onClick={(e) => handleAction('view')}>
          <VisibilityIcon />
        </IconButton>
      </Tooltip>
      ...
      <Tooltip title="Abrir formulário de alteração dos dados deste registro">
        <IconButton aria-label="alterar" onClick={(e) => handleAction('edit')}>
          <EditIcon />
        </IconButton>
      </Tooltip>
      {moreActions != null && (
