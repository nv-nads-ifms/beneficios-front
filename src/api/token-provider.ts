...
export const createAuthProvider = <T>({
    ...
    const authFileFetch = async (input: RequestInfo, init?: RequestInit): Promise<Response> => {
        const token = await tp.getToken();
    ...
    return [useAuth, authFetch, authFileFetch, login, logout] as [typeof useAuth, typeof authFetch, typeof authFileFetch, typeof login, typeof logout];
};
...
export const [useAuth, authFetch, authFileFetch, login, logout] = createAuthProvider<{
    accessToken: string;
    refreshToken: string;
}>({});